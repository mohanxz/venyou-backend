...
import com.venyou.service.dto.HallAlternativeDTO;
...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingResponseDTO createBooking(BookingRequestDTO bookingRequestDTO) {
        ...
        // Calculate buffer times
        int bufferHours = getBufferHoursForEventType(bookingRequestDTO.getEventType());
        ...
        booking.setEventType(bookingRequestDTO.getEventType());
        booking.setEventStartDate(bookingRequestDTO.getEventStartDate());
        ...
        booking.setAdditionalInput(bookingRequestDTO.getAdditionalInput());
        ...
        if (bookingRequestDTO.getBookingType() == BookingType.ADVANCE_PAYMENT) {
    ...
    public BookingResponseDTO processPayment(PaymentRequestDTO paymentRequestDTO) {
        ...
        // Create invoice if payment is complete
    ...
    public void checkAndProcessDuePayments() {
        ...
        List<BookingAdvanceDetails> duePayments = bookingAdvanceDetailsRepository
                .findByFullPaymentDueDateBeforeAndFinalPaymentStatus(today, BookingAdvanceDetails.PaymentStatus.PENDING);
        ...
        for (BookingAdvanceDetails advanceDetails : duePayments) {
            ...
            if (booking.getStatus() != Status.ADVANCE_PAID) {
                continue; // Only process bookings that had advance paid
            }
            
            // Mark as failed
            booking.setStatus(Status.FAILED);
            
            // Release the hall availability
    ...
    private void validateBookingRequest(BookingRequestDTO bookingRequestDTO) {
        if (bookingRequestDTO.getEventEndDate().isBefore(bookingRequestDTO.getEventStartDate())) {
        ...
        long daysUntilEvent = ChronoUnit.DAYS.between(LocalDate.now(), bookingRequestDTO.getEventStartDate());
    
        if (daysUntilEvent <= 10 && bookingRequestDTO.getBookingType() == BookingType.ADVANCE_PAYMENT) {
            throw new BookingConflictException(
                "Events within 10 days require full payment. Please change bookingType to FULL_PAYMENT"
            );
        }
    }
    ...
    private AvailabilityCheckDTO mapToAvailabilityCheckDTO(BookingRequestDTO bookingRequestDTO) {
        ...
        dto.setEventType(bookingRequestDTO.getEventType());
        dto.setStartDate(bookingRequestDTO.getEventStartDate());
    ...
    private void handleAdvancePaymentBooking(Booking booking) {
        ...
        advanceDetails.setAdvancePaymentDate(null); // Will be set when payment is made
    ...
    private void handleFullPaymentBooking(Booking booking) {
        booking.setStatus(Status.PAYMENT_PENDING); // Waiting for full payment
    }
...
private InvoiceResponseDTO mapToInvoiceResponseDTO(Invoice invoice) {
    ...
    public PriceCalculationResponse calculatePrice(Long hallId, PriceCalculationRequest request) {
        ...
        // Use the same format as frontend
        ...
        // Calculate based on 12-hour blocks
